...
NOTES
We're using expo to handle distribution, you can develop with expo by using ```npx expo start --tunnel``` and use the expo go app to scan the QR code that opens (your phone must be on the same network that you're hosting this server on)
- use --tunnel option when starting local dev server, default didn't work for me but might for you


App.tsx is the entry point defined in node_modules/expo/AppEntry.js, so don't change the name of this.
app.json is used to tell expo which static assets it should bundle with your app
